			...
			for (int i = 0; i < s.length(); ++i) {
				for (int y = 0; y < 5; ++y) {
					for (int key : rules.keySet()) {
						if (rules.get(key).get(0).length()==1) {
					...
					for (int y = 0; y < 5; ++y) {
						for (int key : rules.keySet()) {
							...
							for (String rule : rules.get(key)) {
								...
								if (rp.length==2) {
									for (int k = i; k < j; ++k) {
										dp[i][j][key] |= dp[i][k][Integer.parseInt(rp[0])]&&dp[k+1][j][Integer.parseInt(rp[1])];
									}
								}
								if (rp.length==3) {
									for (int k = i; k < j; ++k) {
										for (int l = k; l < j; ++l) {
											dp[i][j][key] |= dp[i][k][Integer.parseInt(rp[0])]&&dp[k+1][l][Integer.parseInt(rp[1])]&&dp[l+1][j][Integer.parseInt(rp[2])];
										}
									}
								}
								if (rp.length>3) {

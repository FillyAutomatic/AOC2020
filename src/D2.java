    ...
    public static void main(String[] args) throws IOException {
        ...
        while (sc.hasNextLine()) {
            ...
            if (fds.get("hgt").matches("\\d{2}in")) {
                if (Integer.parseInt(fds.get("hgt").substring(0, 2)) < 59
                        || Integer.parseInt(fds.get("hgt").substring(0, 2)) > 76)
                    continue;
            } else if (fds.get("hgt").matches("\\d{3}cm")) {
                if (Integer.parseInt(fds.get("hgt").substring(0, 3)) < 150
                        || Integer.parseInt(fds.get("hgt").substring(0, 3)) > 193)
                    continue;
            } else {
                continue;
            }
